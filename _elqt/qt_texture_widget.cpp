...
#include <systems/batch_system.h>
#include <systems/loader_system.h>
#include <systems/button_system.h>

#include <qtimer.h>

namespace el
{
	QElangView* QElangTextureWidget::view(){ return ui.view; }
	...
	QElangTextureWidget::QElangTextureWidget(QWidget* parent)
		...
		connect(ui.view, &QElangView::sig_ResizeGL, [&](int w, int h) {
			setStage(mStage);
			mWinWidth = w;
			mWinHeight = h;
		...
		connect(ui.view, &QElangView::sig_PaintGL, [&]() {
			setStage(mStage);
			glClearDepth(1.0f);
			...
			//updateBasicSprites();
			//batchBasic2DComponents();

			batchDebuggingComponents(mGadgetPainter, color(0, 255, 0, 255));
	...
	void QElangTextureWidget::connectMouseInput() {
		connect(ui.view, &QElangView::sig_MousePress, [&]() {
			setStage(mStage);
		...
		connect(ui.view, &QElangView::sig_ScrollWheel, [&]() {
			setStage(mStage);
			float val = (5.0f / 6.0f);
